...
def test_single_image(model_path, image_path, save_name="test_result_finetuned.png"):
    ...
    if not os.path.exists(model_path):
        ...
        print("تأكد أن التدريب قد قام بحفظ الملف الجديد بنجاح.")
        return
    ...
    # 4. تجهيز قناة التفاصيل (L) عالية الدقة للدمج الذكي
    img_lab_full = rgb2lab(img_rgb_full).astype("float32")
    L_high_res = img_lab_full[:, :, 0] 

    # 5. تجهيز النسخة المصغرة (128x128) لتدخل إلى الموديل
    ...
    # 6. التنبؤ بالألوان (باستخدام AMP كما تم التدريب)
    print("🎨 جاري تلوين الصورة بالنسخة الجديدة (Finetuned)...")
    with torch.no_grad():
        with torch.amp.autocast('cuda'):
            # تحويل الأرقام لـ 32-بت ليتعامل معها OpenCV بسلاسة
    ...
    plt.title("AI Colorization (Finetuned)")
    plt.axis('off')
...
if __name__ == "__main__":
    # ⬅️ التعديل هنا: استخدام ملف الأوزان الجديد
    MODEL_FILE = "unet_color_Finetuned_100_epochs.pth" 
    
    # 👇 ضع مسار الصورة التي تريد اختبارها هنا (جرب صورة من الداتا الجديدة!)
